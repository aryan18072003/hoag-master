...
def hoag_step(theta, y, physics_op, model, loss_fn, mask,
    ...
    #   We use finite differences because the physics operator (grid_sample)
    #   doesn't support second derivatives (create_graph=True fails).
    #
    #   For each hyperparameter θ_j:
    #     (∂²h/∂w∂θ_j)ᵀ · q ≈ [∇_w h(w*, θ+ε·eⱼ) - ∇_w h(w*, θ-ε·eⱼ)]ᵀ · q / (2ε)
    #
    #   This computes the cross-derivative using only first-order gradients.
    ...
    # DIRECT TERM:
    #   ∂g/∂θ — does the outer loss depend directly on θ?
    #   For our problem: g = DiceBCE(UNet(w*), mask) doesn't depend on θ directly,
    #   so this is zero. But we include it for generality.
    direct_grad = torch.zeros_like(theta)
    
    # FINAL HYPERGRADIENT (original hoag.py line 175)
