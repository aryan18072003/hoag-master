...
IMPORTANT NOTE ON HVP IMPLEMENTATION:
    The "exact" HVP via double-backpropagation (create_graph=True) does NOT work
    with DeepInv's Tomography operator because it uses torch.grid_sample internally,
    which does not support second derivatives (aten::cudnn_grid_sampler_backward).
    
    Therefore, we use FINITE-DIFFERENCE HVP as a robust alternative:
        H·v ≈ (∇h(w + εv) - ∇h(w - εv)) / (2ε)
    
    This only requires first-order derivatives (single backprop), avoiding the issue.
    Finite-difference HVP is widely used in practice and is the standard approach
...
def hessian_vector_product(inner_loss_fn, w_star, theta, y, physics_op, v, fd_eps=1e-4):
    ...
    We use central finite differences instead of exact autograd because
    DeepInv's Tomography operator uses grid_sample, which does NOT support
    double backpropagation (second derivatives).
    
    Formula (Central Finite Differences):
        H·v ≈ (∇h(w + ε·v) - ∇h(w - ε·v)) / (2ε)
    
    This is O(ε²) accurate and only requires first-order gradients.
    
    Why not exact HVP (double backprop)?
        PyTorch's grid_sample (used in DeepInv's Radon transform) raises:
        "RuntimeError: derivative for aten::cudnn_grid_sampler_backward is not implemented"
        Finite differences bypass this by only computing first derivatives.
    
    Args:
        inner_loss_fn: Inner loss function h(w, θ, y, A)
        w_star: Current inner solution w*
        theta: Current hyperparameters (DETACHED — no gradient tracking through θ)
        y: Measurement data
